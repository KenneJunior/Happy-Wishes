...
assert(configRM.maxActiveParticles === 12, "Reduced motion desktop limits to 12 stationary particles (8-12 range)");
assert(configRM.spawnIntervalMs >= 1600 && configRM.spawnIntervalMs <= 2200, "Reduced motion enforces desktop spawn interval in 1600-2200ms range");
assert(configRM.burstCount === 8, "Reduced motion desktop sets controlled celebration burst count to 8");
assert(configRM.enable3DTransforms === false, "Reduced motion disables 3D transforms");
assert(configRM.allowNearTier === false, "Reduced motion disables near tier to prevent obstructing foreground");

// Mobile Reduced Motion
DeviceManager.isMobile = true;
let configRMMobile = getDeviceParticleConfig('heavy');
assert(configRMMobile.maxActiveParticles === 8, "Reduced motion mobile limits to 8 stationary particles (4-8 range)");
assert(configRMMobile.spawnIntervalMs >= 2200 && configRMMobile.spawnIntervalMs <= 3000, "Reduced motion enforces mobile spawn interval in 2200-3000ms range");
assert(configRMMobile.burstCount === 5, "Reduced motion mobile sets controlled celebration burst count to 5");
DeviceManager.isMobile = false;
...
assert(gradParticle.style.transform === 'none', "Reduced motion transform is none");

// Test spawnOccasionParticleBurst under reduced motion (controlled stationary celebration burst)
clearFloatingParticles();
spawnOccasionParticleBurst(50, 50, 'birthday');
assert(container.childElementCount <= 8, "spawnOccasionParticleBurst triggers controlled stationary celebration particles under reduced motion");
